...
object Expressions {
  ...
  def readPipe[F[_]](implicit t: P[_]) =
    P("Read(" ~ STRING ~ ")").map(_ => ReadPipe[F]())

  def invokePipe[F[_]: MonadState[*[_], BotEnv[F]]](implicit t: P[_]) =
    P("Invoke(" ~ STRING ~ ")").map(invokeName => InvokePipe(invokeName))

  def apipes[F[_]: MonadState[*[_], BotEnv[F]]](implicit t: P[_]) =
    P( readPipe[F] | printPipe[F])

  def pipes[F[_]: MonadState[*[_], BotEnv[F]]](implicit t: P[_]) = P( " => " ~ apipes[F])

  def pipeline[F[_]: MonadState[*[_], BotEnv[F]]](implicit t: P[_]) =
